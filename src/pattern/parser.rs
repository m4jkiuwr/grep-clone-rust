    ...
    fn from(it: &mut Chars<'a>) -> ReAutomata<'a> {
        ...
        while let Some(x) = it.next() {
            let next_elem: Box<ReState<'a>> = match x {
                '\\' => match it.next() {
                    Some('w') => Box::new(AlphaNum::new(reference, reference + 1)),
                    Some('d') => Box::new(Digit::new(reference, reference + 1)),
                ...
                '|' => {
                    ...
                    states = vec![Box::new(OrElse::new(lhs, rhs, 0, 1))];
                    break;
                }
...
mod tests {
    ...
    #[test]
    fn test() {
        let mut pat = r"^I see (\d (cat|dog|cow)(, | and )?)+$".chars();
        let x = ReAutomata::from(&mut pat);
    }
}
